...
def property(itx_file: IO[str]) -> dict[str, str | float | NDArray]:
...
def tune(itx_file: IO[str], angle_correction: float = 0) -> list[str]:
    ...
    this means that the wave name starts with "Spectrum".  Unfortunately, this way does not provide the
    unique name.  The use of the "Spectrum ID" is more better, because the uniqueness is guaranteed
    in the single sle file.
