...
class TestARPES(object):
    ...
    def test_axis(self):
        ...
        np.testing.assert_allclose(np.array([0., 1.153846, 2.307692]),
                                   self.arpes041.angle_degs[0:3],
                                   rtol=1e-06)
        np.testing.assert_allclose(np.array([19.615385, 20.769231, 21.923077]),
    ...
    def test_angle_and_energy_shift(self):
        ...
        np.testing.assert_allclose(np.array([4., 5.153846, 6.307692]),
                                   self.arpes041.angle_degs[0:3],
                                   rtol=1e-06)
        self.arpes041.energy_shift(-3.0)
        np.testing.assert_allclose(np.array([2.1, 2.11, 2.12, 2.13]),
    ...
    def test_angle_to_k(self):
        ...
        kmax = 0.512 * np.sqrt(energymax) * np.sin(
            np.max(self.arpes041.angle_degs) * np.pi / 180)
        kmin = 0.512 * np.sqrt(energymin) * np.sin(
            np.min(self.arpes041.angle_degs) * np.pi / 180)
